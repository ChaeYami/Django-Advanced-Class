...
class ArticleList(APIView):
    # READ
    def get(self, request, format=None):  # => if request.method == 'GET':
...
class ArticleDetail(APIView):
    # 상세페이지
    def get(self, request, article_id, format=None):  # request.method == 'GET':
        article = get_object_or_404(Article, id=article_id)  # 데이터 가져오기
        serializer = ArticleSerializer(article)  # 데이터 보여주기
        return Response(serializer.data)

    # 수정
    def put(self, request, article_id, format=None):  # request.method == 'PUT'
        article = get_object_or_404(Article, id=article_id)  # 데이터 가져오기
        serializer = ArticleSerializer(
            article, data=request.data
        )  # 데이터 받아오기 (post 와 get 이 섞인 느낌!)
        if serializer.is_valid():  # 유효성 검사 통과
            serializer.save()  # 받아온 데이터 db에 저장
    ...
    # 삭제
    def delete(self, request, article_id, format=None):  # request.method == 'DELETE':

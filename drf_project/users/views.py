...
class UserView(APIView):
    def post(self, request): # => request.method == 'POST':
        ...
        if serializer.is_valid(): # 유효성 검사 통과
            ...
            return Response({"message" : "가입완료!"}, status = status.HTTP_201_CREATED) # 정상 생성 (등록) 상태
        else: # 유효성 검사 만족 X -> 입력값에 문제
            return Response({"message" : f"${serializer.errors}"}, status = status.HTTP_400_BAD_REQUEST) # 에러를 반환, 잘못된 요청 상태
